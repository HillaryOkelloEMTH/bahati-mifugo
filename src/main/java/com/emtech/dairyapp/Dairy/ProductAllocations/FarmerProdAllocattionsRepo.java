...
public interface FarmerProdAllocattionsRepo extends JpaRepository<FarmerProductAllocations,Long> {
    ...
    @Query(value = "SELECT a.id,f.farmer_no,f.username,a.no_of_cows as noOfCows,a.heat_start_date as heatstartDate,a.type,p.name as product,a.status,a.amount as amount,a.quantity as quantity,DATE(a.allocatio_date) as allocationDate,CAST(a.allocatio_date as time) as time ,a.allocatedby as allocatedBy ,a.payment_status as paymentStatus,a.revoke_status as  revokeStatus, a.service_status as serviceStatus, a.requested_on as requestedOn, a.resolved_on as resolvedOn, a.resolved_by as resolvedBy from farmer_product_allocations a join farmer f  on f.farmer_no=a.farmer_no join  product p on p.id =a.product_id and a.type = 'Service' and a.service_status <> 'Pending'",nativeQuery = true)
    List<Services> fetchAllServicesHistoryApplications();

    @Query(value = "SELECT a.id,f.farmer_no,f.username,a.no_of_cows as noOfCows,a.heat_start_date as heatstartDate,a.type,p.name as product,a.status,a.amount as amount,a.quantity as quantity,DATE(a.allocatio_date) as allocationDate,CAST(a.allocatio_date as time) as time ,a.allocatedby as allocatedBy ,a.payment_status as paymentStatus,a.revoke_status as  revokeStatus, a.service_status as serviceStatus, a.requested_on as requestedOn, a.resolved_on as resolvedOn, a.resolved_by as resolvedBy from farmer_product_allocations a join farmer f  on f.farmer_no=a.farmer_no join  product p on p.id =a.product_id and a.type = 'Service' and a.farmer_no= :memberNo and a.service_status <> 'Pending'",nativeQuery = true)
    List<Services> fetchAllServicesHistoryApplicationsByFarmerNo(Long memberNo);

    @Query(value = "SELECT a.id,f.farmer_no,f.username,a.no_of_cows as noOfCows,a.heat_start_date as heatstartDate,a.type,p.name as product,a.status,a.amount as amount,a.quantity as quantity,DATE(a.allocatio_date) as allocationDate,CAST(a.allocatio_date as time) as time ,a.allocatedby as allocatedBy ,a.payment_status as paymentStatus,a.revoke_status as  revokeStatus, a.service_status as serviceStatus, a.requested_on as requestedOn, a.resolved_on as resolvedOn, a.resolved_by as resolvedBy from farmer_product_allocations a join farmer f  on f.farmer_no=a.farmer_no join  product p on p.id =a.product_id and a.type = 'Service' and a.farmer_no= :memberNo and DATE(a.allocatio_date) between :fromDate and :toDate and a.service_status <> 'Pending'",nativeQuery = true)
    ...
    @Query(value = "SELECT a.id,f.farmer_no,f.username,a.no_of_cows as noOfCows,a.heat_start_date as heatstartDate,a.type,p.name as product,a.status,a.amount as amount,a.quantity as quantity,DATE(a.allocatio_date) as allocationDate,CAST(a.allocatio_date as time) as time ,a.allocatedby as allocatedBy ,a.payment_status as paymentStatus,a.revoke_status as  revokeStatus, a.service_status as serviceStatus, a.requested_on as requestedOn, a.resolved_on as resolvedOn, a.resolved_by as resolvedBy from farmer_product_allocations a join farmer f  on f.farmer_no=a.farmer_no join  product p on p.id =a.product_id and a.type = 'Service' and YEAR(a.allocatio_date) = :year and MONTH(a.allocatio_date) = :month",nativeQuery = true)
    ...
    @Query(value = "SELECT a.id,f.farmer_no,f.username,a.no_of_cows as noOfCows,a.heat_start_date as heatstartDate,a.type,p.name as product,a.status,a.amount as amount,a.quantity as quantity,DATE(a.allocatio_date) as allocationDate,CAST(a.allocatio_date as time) as time ,a.allocatedby as allocatedBy ,a.payment_status as paymentStatus,a.revoke_status as  revokeStatus, a.service_status as serviceStatus, a.requested_on as requestedOn, a.resolved_on as resolvedOn, a.resolved_by as resolvedBy from farmer_product_allocations a join farmer f  on f.farmer_no=a.farmer_no join  product p on p.id =a.product_id and a.type = 'Service' and DATE(a.allocatio_date) between :fromDate and :toDate and a.service_status = 'Pending'",nativeQuery = true)
