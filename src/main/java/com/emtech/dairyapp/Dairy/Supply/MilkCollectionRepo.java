...
public interface MilkCollectionRepo extends JpaRepository<MilkCollections, Long> {
    ...
    @Query(value = "SELECT f.first_name ,f.last_name ,f.farmer_no,c.id, c.collection_number as collectionCode ,c.event,c.current_price as currentPrice,c.product_type as productType,c.payment_status as paymentStatus,f.id as farmerId,u.user_name as collector,f.username as farmer ,c.amount,c.quantity,c.collection_date,r.route as route from collections c join users u on c.collector_id =u.id join farmer f on f.farmer_no=c.farmer_no join route r on r.id=c.route_fk   where c.collector_id =:collectorId and c.farmer_no like :farmerNo and c.event= 'Collection'  and DATE(collection_date) BETWEEN :from and :to ", nativeQuery = true)
    List<CollectionsData> getFilteredCollections(Long collectorId, String farmerNo,  String from, String to);
    ...
    @Query(value = "SELECT c.session,,c.id, c.collection_number as collectionCode ,c.event,c.current_price as currentPrice,c.product_type as productType,c.payment_status as paymentStatus,u.user_name as collector,c.amount,c.quantity,c.collection_date,r.route as route,p.name as pickUpLocation from collections c join users u on c.collector_id =u.id  join route r on r.id=c.route_fk join pick_up_locations p on p.id =r.location_id  where c.collector_id =:collectorId order by c.collection_date", nativeQuery = true)
...
//    Long getMaxVaue();
    ...
    @Query(value = "SELECT f.farmer_no, f.username,COALESCE(SUM(c.amount), 0.0) AS collectionAmount \n" +
            "FROM collections c \n" +
