...
List<AnalyticsData> getCollectorsPerCollector(String date);
    @Query(value = "SELECT sum(c.amount) as amount ,CAST(SUM(c.quantity)as DECIMAL(5,2)) as quantity,r.route as route,count(*) as ColectionsCount from collections c join route r on r.id=c.route_fk where DATE(c.collection_date)=:date GROUP BY c.pick_up_location",nativeQuery = true)
    List<AnalyticsData> getCollectorsPerLocation(String date);
    @Query(value = "SELECT c.amount ,c.quantity ,c.current_price ,DATE(c.collection_date) as date,c.session,c.collection_number ,u.user_name as collector,r.route as route from collections c join users u on u.id=c.collector_id join route r on r.id=c.route_fk where DATE(c.collection_date) =:date",nativeQuery = true)
