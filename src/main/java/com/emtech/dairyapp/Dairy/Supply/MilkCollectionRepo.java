...
public interface MilkCollectionRepo extends JpaRepository<MilkCollections,Long> {
    ...
    @Query(value = "SELECT sum(c.quantity) as quantity,u.user_name as username,sum(c.amount) as amount FROM collections c join users u on u.id=c.collector_id WHERE DATE(c.collection_date) = CURDATE()",nativeQuery = true)
