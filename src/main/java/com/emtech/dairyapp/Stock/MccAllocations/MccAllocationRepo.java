...
public interface MccAllocationRepo extends JpaRepository<MccAllocation, Long> {
    ...
    @Query(value = "select ma.product_id, ma.allocated_on, ma.updated_on, p.price,ma.stock, p.name, p.description, p.category, p.type, p.category_id, pp.selling_price from mcc_allocation ma join product p on ma.product_id=p.id join product_price pp on ma.product_id=pp.product_id where ma.location_id = :locationId  group by ma.id", nativeQuery = true)
